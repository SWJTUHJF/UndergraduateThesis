...
def first_insight():
    ...
    ax.set_ylim(85000, 102000)  # 设置Y轴范围
    ax.set_yticks([85000, 88866, 90000, 93525, 95000, 100000])  # 设置刻度间隔为5000
...
def generate_data():
    ...
    lower_problem(nd, lower_gap=1e-5, ppn=[3, 1, 2, 2])
    link_flow_npue = [link.flow for link in nd.Link[1:]]
    link_cost_npue = [link.cost for link in nd.Link[1:]]
...
def cost_heatmap():
    ...
    nd = Network("Nguyen-Dupuis", sst=1)
    lower_problem(nd, lower_gap=1e-5, ppn=[3, 1, 2, 2])
    npue_ue = np.zeros(shape=(len(nd.Node[1:]), len(nd.Node[1:])))
    npue_so = np.zeros(shape=(len(nd.Node[1:]), len(nd.Node[1:])))
    for i in range(1, len(nd.Link)):
        cost_ue = net_ue.LINKS[i].cost
        cost_so = net_so.LINKS[i].get_specific_cost(net_so.LINKS[i].flow)
        cost_npue = nd.Link[i].cost
        tail, head = nd.Link[i].tail.node_id, nd.Link[i].head.node_id
